        ...
        {React.createElement(() => {
            ...
            const handleChange = ({ selected }) => {
                if (!Array.isArray(selected)) {
                    setValue(selected ? selected.text : '');
                }
            };
...
<Preview>
    <div style={{ height: 180 }}>
        {React.createElement(() => {
            ...
            const handleChange = (payload) => {
                const value = payload.selected.length
                    ? payload.selected.map(option => option.text).join(', ') + ', '
                    : '';
                setValue(value);
            };
            const inputValues = value.split(',').map(v => v.trim()).filter(v => v);
            const selected = options.filter(option => inputValues.includes(option.text.trim()));
            const filteredOptions = inputValues.length === selected.length ?
                options :
                options.filter(option => {
                    return selected.includes(option) || matchOption(option, inputValues[inputValues.length - 1])
                });
            return (
                <Autocomplete
                    options={filteredOptions}
