...
<Story name='Chart.Bar'>
    {React.createElement(() => {
        ...
        const marginLeft = number('margin: { left }', 20, {}, composeChartId);
        const marginRight = number('margin: { right }', 50, {}, composeChartId);
        ...
        const yAxisTickMargin = number('tickMargin', 10, {}, yAxisId);
        ...
        const lineType = select('type', linesType, 'monotone', lineId);
        const lineStrokeWidth = number('strokeWidth', 2, {}, lineId);
        const lineStroke = text('stroke', 'var(--color-static-status-blue)', lineId);
        const lineName = text('name', 'остаток', lineId);
        const lineScale = number('dotSettings: { scale }', 1.5, {}, lineId);
        const lineInitScale = number('dotSettings: { initScale }', 1, {}, lineId);
        const lineWidth = number('dotSettings: { width }', 18, {}, lineId);
        const lineDot = boolean('dot', true, lineId);
        const lineInheritStroke = boolean('inheritStroke', true, lineId);
        ...
        const barFillFirst = text('fill', 'var(--color-static-graphic-persimmon)', barIdFirst);
        const barNameFirst = text('name', 'расход', barIdFirst);
        ...
        const barFillSecond = text('fill', 'var(--color-static-graphic-green-jungle)', barIdSecond);
        const barNameSecond = text('name', 'приход', barIdSecond);
        ...
        return (
            <div>
                <div style={{ marginBottom: '20px', top: 25, width: '100%', maxWidth: '1050px', height: '600px' }}>
...
<Story name='Chart.Line'>
    {React.createElement(() => {
        ...
        const initMarginBottom = number('initMargin: { bottom }', 20, {}, composeChartId);
        const marginLeft = number('margin: { left }', 20, {}, composeChartId);
        const marginRight = number('margin: { right }', 70, {}, composeChartId);
        const barCategoryGap = text('barCategoryGap', '30%', composeChartId);
        const barGap = number('barGap', 4, {}, composeChartId);
        ...
        const yAxisTickCoiunt = number('tickCount', 6, {}, yAxisId);
        const yAxisMinTickGap = number('minTickGap', 5, {}, yAxisId);
        const yAxisTickMargin = number('tickMargin', 2, {}, yAxisId);
        const yAxisLine = boolean('axisLine', false, yAxisId);
        ...
        const lineStrokeWidth = number('strokeWidth', 2, {}, lineId);
        const lineStroke = text('stroke', 'var(--color-static-status-blue)', lineId);
        const lineName = text('name', 'текущий период', lineId);
        const lineScale = number('dotSettings: { scale }', 1.5, {}, lineId);
        const lineInitScale = number('dotSettings: { initScale }', 0, {}, lineId);
        const lineWidth = number('dotSettings: { width }', 18, {}, lineId);
        const lineStrokeDasharray = object('strokeDasharray', [10, 10], lineId);
        const lineInheritStroke = boolean('inheritStroke', true, lineId);
        const lineDot = boolean('dot', true, lineId);
        ...
        const areaIcon = select('icon', iconTypes, 'filledCircle', lineId);
        const areaZIndex = number('zIndex', 100, {}, areaId);
        const areaOffset = number('offset', 0.1, {min: 0, max: 1, step: 0.1}, areaId);
        const areaColor_1 = text('gradient color above', 'rgba(0, 112, 224, 0.2)', areaId);
        const areaColorOpacity_1 = number('gradient opacity of color above', 1, {min: 0, max: 1, step: 0.1}, areaId);
        const areaColor_2 = text('gradient color below', 'rgba(0, 128, 255, 0)', areaId);
        const areaColorOpacity_2 = number('gradient opacity of color below', 1, {min: 0, max: 1, step: 0.1}, areaId);
        const areaType = select('type', linesType, 'monotone', areaId);
        const areaStrokeWidth = number('strokeWidth', 2, {}, areaId);
        const areaStroke = text('stroke', 'var(--color-static-status-blue)', areaId);
        const areaName = text('name', 'предыдущий период', areaId);
        const areaScale = number('dotSettings: { scale }', 1.5, {}, areaId);
        const areaInitScale = number('dotSettings: { initScale }', 0, {}, areaId);
        const areaWidth = number('dotSettings: { width }', 18, {}, areaId);
        const areaStrokeDasharray = object('strokeDasharray', [1, 0], areaId);
        const areaDot = boolean('dot', true, areaId);
