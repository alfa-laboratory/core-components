...
<Story name='Песочница'>
    {React.createElement(() => {
        const [variant, setVariant] = useState({ value: 'success', text: 'Успешный кейс'});
        ...
        return (
            <div>
                <Select
                    options={[
                        { value: 'success', text: 'Успешный кейс'},
                        { value: 'error', text: 'Кейс с ошибкой'},
                        { value: 'fatal', text: 'Кейс с критичной ошибкой'}
                    ]}
                    onChange={(_, { selected }) => {
                        setVariant(selected)
                    }}
