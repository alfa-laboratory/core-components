...
<Story name='Amount'>
    {React.createElement(() => {
        ...
        const majorBold = select('majorBold', [undefined, true, false], undefined);
        const minorBold = select('minorBold', [undefined, true, false], undefined);
        const minorColor = select('minorColor', ['transparent', 'inherit'], 'transparent');
