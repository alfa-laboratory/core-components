...
```tsx
import { KeyboardFocusable, useKeyboardFocus } from '@alfalab/core-components-keyboard-focusable';
```
...
Для получения такого поведения — можно воспользоваться компонентом `<KeyboardFocusable />` или хуком `useKeyboardFocus`.
