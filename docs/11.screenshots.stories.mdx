...
**Версия `Node.JS` должна быть >=14.2.0, иначе может быть проблема с установкой зависимостей!**
...
где делается скришот этой страницы.
...
describe(
    ...
    screenshotTesting({ cases, it, beforeAll, afterAll, expect }),
);
```

**_Функция `getComponentScreenshotTestCases` вернет все возможные комбинации пропсов.
